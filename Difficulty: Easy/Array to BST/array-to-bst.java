...
class Solution {
    ...
    private Node constructBST(int[] nums, int left, int right) {
        ...
        Node root = new Node(nums[mid]); // Create a new node
        
        root.left = constructBST(nums, left, mid - 1);  // Recursively build left subtree
        root.right = constructBST(nums, mid + 1, right); // Recursively build right subtree
